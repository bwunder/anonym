  ...
  chooseImageId: async (message='SQL Image', filterfn) => { 
    return new Promise( async (resolve, reject) => {
      ...
      return prompt([{
        ...
        choices: choices,
        filter: filterfn,
        default: await catalog.latestImage() 
      }])
      .then( answer => {
            ...
            if (catalog.sql.ContainerInfos.has(instanceId)) {
              ...
              if (instanceId===catalog.sql.Instance) { 
                if (catalog.sql.ContainerInfos.get(instanceId).State==='running') {
                  ...
                  if (!(await lines.getLast()).line.includes('stop')) {
                    if (await api.confirm(`Start Target Container now?`)) {
                      await catalog.intern(await catalog.startInstance(instanceId))
                    }
                  }
                }  
              } else {
                ...
                if (await api.confirm(`Select a Target SQL Container now?`)) {
                  await catalog.intern(await catalog.chooseInstanceId('all'))
                }
            }
            } else {
              if (catalog.sql.ContainerInfos.size>0) {
    ...
    return new Promise( async(resolve, reject) => {
      try {
        ...
        container.logs(logConfig, async (err, stream) => {
          ...
          await container.modem.demuxStream(stream, logStream, logStream)
          stream.on('end', async () => { 
            ignore=true // stop listening 
            logStream.end
            if (logerrs.length>0) {
          ...
          setTimeout(() => {
            if (stream.readable) {
              stream.emit('end')
              log('warn', `(tailLog) Log stream has timed out\n` +
                          `containerId: ${containerId}\n` +
                          `docker: ${stream.socket.parser.outgoing.path}\n` +            
                          `logConfig: ${format(logConfig)}\n`)
              if (logConfig.since) log('warn', `logConfig.since: ${new Date(logConfig.since).toUTCString()} ${new Date(logConfig.since)}`)
            } else stream.destroy() 
            resolve()
