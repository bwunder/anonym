...
module.exports = catalog = {
  ...
  cmdAttached: async (containerId, command, filter) => {
              ...
              exec.start( (err, stream) => {
                ...
                setTimeout(() => { 
                  finished(stream, (reply, err) => {
                    ...
                    resolve(log('warn', `(cmdAttached) stream has timed out
        ...
        if (await catalog.isHost()) { 
          ...
          if (await catalog.internImages()) {
            if (await catalog.internContainers()) {
              if (!catalog.sql.Instance) {
                if (catalog.sql.ContainerInfos.size===1) {
                  catalog.sql.Instance=catalog.sql.ContainerInfos.keys().next()
                } else if (catalog.listInstances('running').length===1) {
                  catalog.sql.Instance = catalog.listInstances('running')[0]                  
                }
              }
            }
            log('progress', `\n`)            
            if (catalog.sql.ContainerInfos.has(catalog.sql.Instance)) {
              await catalog.internInstance(catalog.sql.Instance)
              // if (containerId===catalog.sql.Instance) { 
              //   if (catalog.sql.ContainerInfos.get(containerId).State!=='running') {
              //     log('target', chalk`{yellowBright Target SQL Server Container not running}: ${containerId}`)
              ...
              // else {
              //   log('target', chalk`{redBright Target SQL Server for CLI's TSQL queries not found}: ${containerId}`)
            ...
            //   if (catalog.sql.ContainerInfos.size>0) {
            //     if (!catalog.sql.Instance) {
            //       log('target', chalk`{yellowBright No Target SQL Container set for CLI's TSQL queries}`)
            //       if (await api.confirm(`Select a Target SQL Container now?`)) {
            //         await catalog.internInstance(await catalog.chooseInstanceId('all'))
            //       }
            //     } else {
            //       log('target', chalk`{redBright Target SQL Container for CLI's TSQL queries not found}: ${catalog.sql.Instance}`)
            //       if (await api.confirm(`Select a Target SQL Container now?`)) {
            //         await catalog.internInstance(await catalog.chooseInstanceId('all'))
            //       }
            //     }
  ...
  internInstance: async (instanceId) => {
    try {
      if (catalog.sql.ContainerInfos.has(instanceId)) {
        ...
        if (catalog.sql.ContainerInfos.get(instanceId).State==='running') {  
          await catalog.openInstance(instanceId)
        }  
        return true
      } else {
        return false
        ...
        return catalog.getProcesses(containerId)
          .then( async (top) => {
            if (top) {
              ...
              // will be cashed & persisted to nedb here so cli can reuse last opened by id
...
log('log', output)
  ...
  stopInstance: async (containerId) => {
    try {
      ...
      log('log', `(stopInstance) ${containerId} ${ucons.get('confirm')}`)
      await catalog.intern()
      return containerId
    }
    catch(err) {
    ...
    return new Promise( async(resolve, reject) => {
      try {
        ...
        container.logs(logConfig, async (err, stream) => {
          ...
          stream.on('end', async () => { 
            // log('confirm', 'stream ending') ??? ends twice ???            
            resolve()
          })
          setTimeout(() => {
            finished(stream, (reply, err) => {
