...
module.exports = catalog = {
  ...
  getImage: async imageId => {
    ...
    return new Promise( async (resolve, reject) => {
      try {
        if (!imageId) {
//log('confirm', `catalog.getInstanceInfo(catalog.sql.Instance) ${catalog.getInstanceInfo(catalog.sql.Instance)}`)
log('confirm', `catalog.getInstanceInfo(catalog.sql.Instance).ImageID ${catalog.getInstanceInfo(catalog.sql.Instance).ImageID}`)
log('confirm', `catalog.sql.Images.get(catalog.getInstanceInfo(catalog.sql.Instance).ImageID) ${catalog.sql.Images.get(catalog.getInstanceInfo(catalog.sql.Instance).ImageID)}`)
    ...
    return new Promise( async (resolve, reject) => {
      try {
        if (await catalog.isHost()) {
          ...
          log('log', chalk`\u2042  {rgb(127,255,0) check engine}`)
          resolve(false)
        }
      }
      catch(err) {
  ...
  listInstanceIds: (status='all') => { // up idle all
      ...
      try {
        if (!imageId) {
          ...
          return getPort()
            .then( async hostPort => {
              let mounts= []
              for (folder of Object.keys(config.docker.bindings)) {
                await api.addFolder(folder)
                mounts.push(config.docker.bindings[folder].mount)
              }
              let env = []
