...
module.exports = catalog = {
  ...
  restartInstance: async (containerId) => {
    try {
      ...
      log('progress', `(restartInstance) ${containerId} ${ucons.get('confirm')} `)
  ...
  startInstance: async (containerId) => {
    try {
      ...
      log('progress', `(startInstance) ${containerId} ${ucons.get('confirm')} `)
  ...
  stopInstance: async (containerId) => {
    try {
      ...
      log('progress', `(stopInstance) ${containerId} ${ucons.get('confirm')} `)
      await catalog.intern()
      return containerId
    }
    catch(err) {
              ...
              switch (true) {
                case (data.includes('SQL Server is now ready')):  // from message_id=17126 
                  ...
                  if (!state.FinishedAt || datestamp > new Date(state.FinishedAt)) {
                    stream.emit('end')
//                    logStream.destroy() // doesn't write any subsequent log rows
                    if (logerrs.length>0) {
