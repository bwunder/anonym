...
//need to define arg as table - !!!better yet TVP (recordset.toTable())
  ...
  .option('--list [grep-args]', `Files in volume 'private' (default)`)
  .option('--mssql', `Create a cert for SQL Server to mssql connections (client side)`)
  .option('--new <name>', chalk`Create and name a private key ({italic name}-key.pem) and sign a new Certificate ({italic name}-cert.pem) into volume 'private'`)
  .option('--sqlserver <container-id>', `Create an SSL query connection cert for a SQL Server (server side)`)
  .option('--sqlpad', `Create an SSL cert for sqlpad`)
  .action( async (args) => {
    switch(true) {
      case (args.options.ca):
        ...
        break
      case (args.options.mssql):
        await api.genCertificate('mssql')
        break
      case (typeof args.options.new=='string'):
        await api.genCertificate(args.options.new)
        break
      case (typeof args.options.list==='string'):
        let grepArgs = await store.lines.getLast()
        await api.elevate(`ls private/ | grep ${grepArgs.line.split(' ').slice(2).join(' ')}`)
        break
      case (args.options.sqlserver):
        await api.genCertificate('sqlsvr')
        break
      case (args.options.list):
      default:
        await api.elevate(`ls private/`)
        break
    }
  })
...
// needs to be able to run on host or in container
  ...
  .option(`--start <ssl-cert-password>`, `Launch SQLPad web server`)
  .option(`--stop`, `Stop SQLPad web server`)
  .option(`-u, --url`, ``)
  .action( async (args) => {
    switch (true) {
      case (typeof args.options.start==='string'):
        api.startSQLPad(args.options.start)
        break
      case (args.options.stop):
        api.sqlpad.kill(1)
        break
