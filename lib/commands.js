...
//need to define arg as table - !!!better yet TVP (recordset.toTable())
  ...
  .option('--ca', `Initialize Self-signing CA in volume 'private'`)
  ...
  .option('--mssql <key-password>', `Create a cert for mssql connections (client sourced creds)`)
  ...
  .option('--sqlserver <key-password>', `Generate an SSL query connection cert for SQL Server (server sourced creds)`)
  .option('--sqlpad <key-password>', `Generate an SSL cert for the sqlpad web server`)
  .action( async (args) => {
    switch(true) {
      case (args.options.all):
      ...
      case (typeof args.options.mssql==='string'):
        // openssl will quietly build mssql node under distinguished name from .cnf
        await api.genCertificate('mssql', args.options.mssql)
        break
      ...
      case (typeof args.options.sqlserver==='string'):
        await api.genCertificate('sqlserver', args.options.sqlserver)
        break
      // sqlpad DN node
      case (typeof args.options.sqlpad==='string'):
        await api.genCertificate('sqlpad', args.options.sqlpad)
        break
