...
const vorpal = new Vorpal()
  ...
  // orchestrated vorpal.exec() only, in a commandInstance the line is always captured too
  // and errors are caught elsewhere so this is a bit redundant
  // vorpal.on('client_command_executed', (data) => {
  //  log.debug(chalk.magenta(`[vorpal client_command_executed]\n${api.format(data)}`))
  // })

  vorpal.command(`about [topic]`, chalk`About ${name} Topics (autocomplete): {italic.bold cli, npm, quickstart, readme, version}`)
  ...
  vorpal.command(`batch`, `Buffered Input as caught by CLI Linereader`)
  ...
  .option(chalk`-s, --sqlcmd`, `Compile as if for sqlcmd submit - command switches, prefix and TSQL Batch `)
  .action( async (args) => {
      switch(true) {
  ...
  //     or use the BCP included in the Target SQL Server container (${name} command: 'instance --bash')`)
  //   log.log(api.bandAid)
  //   callback()
  // })
...
//need to define arg as table - !!!better yet TVP (recordset.toTable())
  ...
  .option(chalk`-c, --container [container-id]`, `Target [or container-id] catalog ContainerInfo`)
  .option(chalk`-i, --image [image-id]`, `Target [or image-id] catalog Image`)
  .option(chalk`-l, --list`, `brief catalog view (default)`)
  .option(chalk`-n, --networks`, `Host ports and bridged IP addresses of all Containers`)
  .option(chalk`-p, --pool [container-id]`, `Existing Target [or container-id] Connection Pool`)
  ...
  vorpal.command(`certificate`, `Self-signing Certificate Authority`)
  .option('--list [filter]', `Files in volume 'private' (default)`)
  .option('--ca', `Create Self-signing CA in volume 'private'`)
  .option('--new <name>', `Create private key and sign a new Certificate into volume 'private'`)
  .action( async (args) => {
    switch(true) {
      ...
      case (args.options.list):
      default:
        await api.elevate(`ls private/ ${typeof args.options.list==='string'?`|${args.options.list}`: ``}`)
        break
    }
  })
...
// ???emacs diff the file and the runtime???
  ...
  .option(chalk`-e, --edit`, `Open 'config.json' file with '${config.cli.ide}'`)
  .option(`-c, --current`, `View config state (default)`)
  ...
  .option('-v, --variables', `View Target Container Environment Variables`)
  .action( async (args) => {
    switch(true) {
      case (args.options.edit):
  ...
  vorpal.command(`instance`, `${sqlName} Docker Containers`)
  .option(chalk`-a, --all`, `All SQL Server Containers`)
  .option(`-b, --bash [container-id]`, `bash shell in a Cataloged Container`)
    ...
    if (await api.isDocker()) {
      ...
      if (containerId) {
        ...
        switch(true) {
          ...
          case (args.options.query):
            sqlArg=`-q "${api.compile(config.sqlcmd.prefix)} ${api.compile()}"`
            log.info(`type 'exit' to close sqlcmd and resume ${name} CLI`)
            usingBatch=true
            break
          default:
            sqlArg=''
