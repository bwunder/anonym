...
module.exports = exports = sqldb = {
  ...
  openPool: async (containerId=api.sqlCatalog.Instance) => {
    ...
    return new Promise(async (resolve, reject) => {
      try {
        let e = new Error() // stack trace that got you here
        ...
        .then( async (top) => {
          if (top.Processes.length===0 || !top.Processes.join().includes(path.join(await api.getEnvVariable(containerId, `$MSSQL_BIN_DIR`), `sqlservr`))) {
