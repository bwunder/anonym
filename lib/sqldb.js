...
module.exports = sqldb = {
  ...
  openPool: async (containerId, options, name) => {
    return new Promise( async (resolve, reject) => {
      try {
        if (config.cli.connectionPool.options.encrypt) {
//TODO & needs creds - (pass in options? or ?)
        ...
        if (options.database && options.database!==result.recordset[0].database) {
          ...
          errorhandler(new Error(`Failed to open Database {bold ${config.cli.connectionPool.database}}, using {bold ${options.database}}`))
        }  
