...
////CORE
const { inspect } = require('util') // error handling
...
module.exports = sqldb = {
  ...
  openPool: async (sqlserver, options) => {
    return new Promise( async (resolve, reject) => {
      try {
...
//TODO & needs creds - (pass in options? or ?)
        ...
        while (openPools[sqlserver]) {
          openPools[sqlserver].close()
          openPools[sqlserver] = undefined 
        }  
        openPools[sqlserver] = new mssql.ConnectionPool(options) 
        openPools[sqlserver].on('error', (err) => {
          errorhandler(err)
        })
        await openPools[sqlserver].connect()
        let request = new mssql.Request(openPools[sqlserver])
        ...
        log('sqldb', `Pool open: db ${options.database} in sql container ${sqlserver}\n`)
  ...
  readLog: (sqlserver, options) => {
    return new Promise( async (resolve, reject) => {
      if (openPools[sqlserver]) {
        ...
        return sqldb.go(`EXEC sys.xp_readerrorlog ${zargs}`, sqlserver)
        .then( results => {
          let i = 0
