...
// client side connector for bash shell (api.cmdAttached(), api.interactiveShell())
const setDockerShell = async (IP) => {
  return new Promise( async (resolve, reject) => {
    try {
      ...
      if (!cliOptions) cliOptions = `-H unix://${config.docker.daemon.socketPath}`
//log('confirm', `(setDockerShell) cliOptions:\n${format(cliOptions)}`)
      resolve(cliOptions)
    }
    catch(err) {
      reject(new Error(`(setDockerShell) error\n${err}`))
    }
  })
}
