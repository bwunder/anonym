...
module.exports = view = {
  dft: out => restyle(`{dim.bgMagenta.inverse default}`, out), 
  descr: out => restyle(`{italic ${description}}`, out),
  delimiter: restyle(`{bold.greenBright ${name}}`, 'cli'),
  introduction: async (cmds, out) => {
    ...
    away you go: 
      ...
      query execution by setting the ${restyle(`bold config.cli.connectionPool.parseJSON}`, out)} key in ${restyle(`{bold ../config/config.json}`, out)} to 
  ...
  markup: async (sourcePath, markdown) => {
    ...
    await api.txtToFile(entities.decode(`<!DOCTYPE html>
      <html>
        <head>
          <title>${doc}</title>
          <link rel="stylesheet" href="../node_modules/highlight.js/styles/${config.browser.style}.css">
