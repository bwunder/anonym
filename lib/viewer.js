...
const restyle = (chalkMarks, out='cli') => {
  ...
  if (out==='www') {
    ...
    // per chalk, "styles (rgb(), hsl(), keyword(), etc.) may not contain spaces between params"
  ...
  introduction: async (cmds, out) => {
    ...
    away you go: 
      ...
      autocompletions. Some examples: 
          ...
          ${restyle(`{italic enter character(s) and two tabs to furter filter possible choices}`, out)}
    ...
    return api.jsonToFile(`<html>
      ...
      <body>
        <div class="outer">
          ...
          ${entities.decode(marked(!markdown? await api.fileToJSON(sourcePath) : markdown))}
        </div>  
      </body>
    </html>`, doc)
    .then(() => doc)    
