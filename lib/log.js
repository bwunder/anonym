...
module.exports = log = {
  format: (input, offset=1) => {
    ...
    let output=''
    const colors = true // process ANSI formatting
    switch (typeof input) {
      case ('undefined'):
      ...
      case ('string'):
        try {
          if (JSON.parse(input)) {
            output = inspect(input, config.cli.log.inspect, colors)
          }
        }
        catch(e) {
          output = input
          ...
          case (Array.isArray(input)):
            input.forEach( result => {
              if (typeof result==='object') {
                output+= inspect(result, config.cli.log.inspect, colors)
              } else {
                output+=result
              }
            })
          ...
          default:
            if (input && input.recordset && input.recordsets) {
              input.recordsets.forEach( rs => {
                output += inspect(rs, config.cli.log.inspect, colors)
              })
            } else {
              output = inspect(input, config.cli.log.inspect, colors)
