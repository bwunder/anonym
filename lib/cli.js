...
vorpal.command(`repo [action]`, chalk`Source control using the simpleGit API\n
  ...
  .action( async (args) => {
    ...
    switch (args.action) {
      case ('commit'):
        ...
        break
      case ('git'):
        let func = await api.choose(Object.keys(repo.git), chalk`{italic simple-git} function`)
