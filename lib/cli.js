...
vorpal.command(`about [topic]`, chalk`Documentation by topic\n
  ...
  Topics:\n 
    ...
    {bold editor}       --help command-line output of {bold.italic config.editor} (now '${config.editor}')
    ...
    {bold version}      Just the version from package.json
  ...
  .option(`-b, --browse`, `Open the document (as HTML) in a browser`)
  .option(`-e, --edit`, chalk`Edit source document with '${config.editor}' ({yellowBright Overrides} {bold.inverse --browse})`)
  .action( async args => {
...
vorpal.command(`batch [action]`, chalk`Primary storage query cache: an array of strings\n
  Actions:\n 
    {bold edit}   open and edit the cached query in {bold ${config.editor}}
    {bold issql}  submit the cached query to verify as TSQL {bold SET NOEXEC ON}
  ...
  .action( async args => {
    if (await catalog.isHost()) {
      ...
      case ('network'):
        ...
        break
      case ('pool'):
        log('log', catalog.listPoolsSummary(await catalog.choosePoolId()))
        break
      case ('remap'):
      ...
      case (undefined):
        if (catalog.sql.Images) {
          await catalog.listImagesSummary() 
          await catalog.listInstancesSummary() 
          catalog.listPoolsSummary() 
        }  
        break
      default:
    ...
    if (await catalog.isHost()) {
      ...
      switch(args.action) {
        ...
        case ('edit'):
          ...
          api.editText(store.pools.get(catalog.choosePoolId()))
...
vorpal.command(`engine [action]`, chalk`dockerd - Self-sufficient runtime for containers\n
  Actions:\n
    {bold force-reload}      reloaded config - may restart the service
    {bold reload}            reloaded config - will not restart the service
    {bold restart}           stop and start the running service
    {bold start}             start the stopped service
    {bold.bgMagenta status}  (${view.dft()}) status of service
    {bold stop}              stop the running service 
    {bold try-restart}       restart the running service else just report success.\n`)   
  ...
  .action( async (args) => {
    ...
    if (!args || ['force-reload', 'reload', 'restart', 'start', 'status', 'stop', 'try-restart'].includes(action)) {
      log('log', await api.setEngine(action))
    } else {
  ...
  .action( async args => {
    if (await catalog.isHost()) {
      ...
      if (catalog.sql.ContainerInfos.has(containerId) &&
        ...
        if (filter) {
          filter = await api.input('find files named like:', filter)
          log('log', await catalog.cmdAttached(containerId, `ls -lb ${folder}`.concat(filter?`| grep ${filter}`:'')))
        } 
      }
    }
  })

//  const simpleGitFunctions = Object.keys(repo.functions).concat(`listSimpleGitFunctions`).sort()
//  const anonymGitFunctions = Object.keys(repo).concat('listAnonymGitFunctions').sort()
//  const gitActions = simpleGitFunctions.concat(anonymGitFunctions).sort() 
  // Actions may call out simplGit functions: 
  //     {bold ${simpleGitFunctions.join('\n')}}
  // or ${view.name()} functions that chain simpleGit functions:
vorpal.command(`git [action]`, chalk`Source control using the simpleGit API\n
    {bold ${Object.keys(repo).concat('listAnonymGitFunctions').sort().join('\n')}}\n`)
  .autocomplete(Object.keys(repo).concat('listAnonymGitFunctions').sort())
  .action( async (args) => {
    let choice
    switch (args.action) {
      case ('listAnonymGitFunctions') :
        choice = await api.choose(anonymGitFunctions, `${view.name()} Git functions`, args.action)
        for (let fn of anonymGitFunctions) {
          log('log', `\t${fn}`)
        }
        break
      case ('listSimpleGitFunctions') :
        choice = await api.choose(simpleGitFunctions, 'simpleGit functions', args.action)
        for (let fn of simpleGitFunctions) {
          log('log', `\t${fn}`)
        }
        break
      case ('new') :
        log('log', `${args.action}: ${repo.functions[args.action].toString()}`)
        break
      case ('newLocal') :
        break
      default :
        log('log', await repo.functions.status())
        break
    }
    if (simpleGitFunctions.includes(args.action)) log('log', `${args.action}: ${repo.functions[args.action].toString()}`)
    if (anonymGitFunctions.includes(args.action)) log('log', `${args.action}: ${format(repo[args.action])}`)
  })
  ...
  .action( async () => {
    // try {
      ...
      let containerId = (typeof args==='undefined' || !args.options.pool)? catalog.sql.Instance: await catalog.choosePoolId() 
  ...
  .action( async args => {
    if (await catalog.isHost()) {
      ...
      case ('available'):
        ...
        break
      case ('delete'):       
        await catalog.intern(await catalog.removeImage())
        break  
      case ('pull'):
        await catalog.intern(await catalog.pullImage())
        break
      case ('run'):
        await catalog.intern(await catalog.runImage())
        break
      case ('full'):
        log('log', await catalog.getImage(await catalog.chooseImageId()))
        break
