  ...
  .action( async args => {
    ...
    switch(args.action) {
      case ('archive'):
        ...
        break
      case ('delete'):
        queryName = await api.choose(await store.templates.names(args.filter), 'Query to delete')
      ...
      case ('develop'):
         ...
         break
      case ('edit'):
        queryName = await api.choose(await store.templates.names(args.filter), 'Query to edit')      
      ...
      case ('get'): // can use to refer back to existing while composing - does not touch Batch
        queryName = await api.choose(await store.templates.names(args.filter), 'query to show')
        log('log', await store.templates.get(queryName))
        break
      case ('import'):
        store.templates.import()
        ...
        break
      case ('put'):
        store.templates.put(await api.input('Name for Query in Store'), api.compile())
        break
      case ('sync'):
      ...
      case ('load'): 
      default:
        queryName = await api.choose(await store.templates.names(args.filter), 'query to load into the Batch', suggestedName)
...
vorpal.catch('[tsql...]', view.descr) 
  ...
  .action( async () => {
    try {
      let txt = (await store.lines..getLast()).line
      if (isBash) { // ALT+b   
