...
.action( async (args) => {
  ...
  switch (args.action) {
    ...
    case ('git'):
      ...
      break
    case ('push'):
      let phrase = await words.getLast('remote')
      if (!phrase) api.input(`Passphrase for ${config.git.origin}`)
      await repo.push(config.git.user, phrase, config.git.origin)
      break
    case ('status'):  
    default:
