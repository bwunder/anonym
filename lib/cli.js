...
const repoFunctions = Object.keys(repo).concat(['gitFunctions', 'repoFunctions']).sort()
...
vorpal.command(`git [action]`, chalk`Source control using the simpleGit API\n
    {bold ${Object.keys(repo).concat(['gitFunctions', 'repoFunctions']).sort()}}
  {bold.underline git functions}\n
    `)
  ...
  .action( async (args) => {
    ...
    switch (args.action) {
      case ('commit'):
        ...
        break
      case ('git'):
        let fn = await api.choose(Object.keys(repo.git), chalk`{italic simple-git} function`)
        // choose a func from list
        // enum and set args from prototype
        // run 
        break
      case ('newLocal'):
        repo.newLocal()
