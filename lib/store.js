...
const collections = {
  ...
  batches: new nedb({
    filename: path.resolve(config.cli.storePath, 'batches.db'),
    timestampData: true,
    autoload: true,
    onload: (err) => {
        if (err) {
          api.log('warn', `(store.batches) error loading ${config.cli.storePath}/batches.db`)
  ...
  configs: new nedb({
    filename: path.resolve(config.cli.storePath, 'configs.db'),
    timestampData: true,
    autoload: true,
    onload: (err) =>{
        if (err) {
          api.log('warn', `(store.commands) error loading ${config.cli.storePath}/commands.db`)
          api.log('error', err.message)
          api.log('debug', err.stack)
        }
      },
  ...
  errors: new nedb({
    filename: path.resolve(config.cli.storePath, 'errors.db'),
    timestampData: true,
    autoload: true,
    onload: (err) =>{
        if (err) {
          api.log('warn', `(store.errors) error loading ${config.cli.storePath}/errors.db`)
          api.log('error', err.message)
          api.log('debug', err.stack)
        }
      },
  ...
  lines: new nedb({
    filename: path.resolve(config.cli.storePath, 'lines.db'),
    inMemoryOnly: false, // false is default
    timestampData: true, // stored as "createdAt": {"$$date": valueOf-nbr} not sure how to query by?
    autoload: true,
    onload: (err) =>{
        if (err) {
          api.log('warn', `(store.lines) error loading ${config.cli.storePath}/lines.db`)
          api.log('error', err.message)
          api.log('debug', err.stack)
        }
      },
  ...
  pulls: new nedb({
    filename: path.resolve(config.cli.storePath, 'pulls.db'),
    inMemoryOnly: false, // false is default
    ...
    onload: (err) =>{
      if (err) {
        api.log('warn', `(store.pulls) error loading ${config.cli.storePath}/pulls.db`)
        api.log('error', err)
      }
    },
    afterSerialization: (data) => {
      // encrypt stream into pulls.db file here - looks like it fires for every character (bit?) of stream
  ...
  npm: new nedb({
    filename: path.resolve(config.cli.storePath, 'npm.db'),
    inMemoryOnly: false, // false is default
    timestampData: true, // false is default adds createAt updatatedAt fields
    autoload: true, // false is default -- no need to call loadDatabase if true
    onload: (err) =>{
      if (err) {
        api.log('warn', `(store.npm) error loading ${config.cli.storePath}/npm.db`)
        api.log('error', err)
      }
    },
    corruptAlertThreshold: 0
  ...
  templates: new nedb({
    filename: path.resolve(config.cli.storePath, 'templates.db'),
    inMemoryOnly: false, // false is default
    timestampData: true, // false is default {{data by key...}, createAt:date, updatedAt:date}
    autoload: true, // false is default -- no need to call loadDatabase if true
    onload: (err) =>{
      if (err) {
        api.log('warn', `(store.templates) error loading ${config.cli.storePath}/templates.db`)
        api.log('error', err)
      }
    },
    corruptAlertThreshold: 0
...
module.exports = exports = store = {
  ...
  configs: {
    ...
    get: function(containerId) {
      ...
      return new Promise( (resolve, reject) => {
        collections.configs.findOne({_id: containerId}).exec(function(err, doc) {
          if (err) return reject(err)
          return resolve(doc)
        })
      })
    ...
    getLastInstanceId: async () => {
      ...
      return new Promise( (resolve, reject) => {
        collections.configs.findOne({}).sort({updatedAt: -1}).exec(function(err, doc) {
          if (err) return reject(err)
    ...
    put: () => {
      ...
      collections.configs.insert({config: config.mssql, _id: api.sqlCatalog.Instance})
    ...
    update: (upsert=true) => {
      ...
      collections.configs.update({_id: containerId}, {_id: containerId, mssql: config.mssql}, { upsert: upsert })
