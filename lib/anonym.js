...
// ?      const remote = `https://${config.git.user}:${Promise.resolve(words.getLast('remote'))}@${config.git.origin}`;
  ...
  for (const key of Object.keys(status)) {
    if (Array.isArray(status[key]) && status[key].length>0) {
      log('log', `git ${format(await git.status())}`)
      pushChanges()
      break
    }
    if ([`ahead`, `behind`].includes(key) && status[key]>0) {
      log('warn', `Local git archive is not in sync with Remote`)
    }  
  }
log('info', await git.listRemote())  
  await attachDocker()
