...
start = async () => {
  try {
    ...
    var changeSummary = await git.status()
    for (const key of Object.keys(changeSummary)) {
      if (Array.isArray(changeSummary[key]) && changeSummary[key].length>0) {
        await commit()
        break
      }
    }
    // changeSummary = await git.status()
    // if (changeSummary.ahead>0 && !changeSummary.behind) { 
    //   return push()
    // } else if (!changeSummary.ahead && changeSummary.behind>0) { 
    //   return fetch()  
    // } else {
    //   return merge()  
    // } 
