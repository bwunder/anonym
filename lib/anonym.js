...
const runSQLPad = require('../config/config.json').sqlpad.runAtStartup
...
return api.attachDocker()
.then( async () => {
  await repo.commit()
  log('log', `git repo aftr ${format(await repo.git.status())}`)
  if (runSQLPad) await sqlpad.map()
  cli.show()
})
.catch( err => {
  if (err.fileName) {
