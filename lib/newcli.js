...
const { batch, fileToJSON } = require('../lib/api')
...
process.stdin.on('keypress', key => {
  if (!key.value && key.meta) { // only blank when newline
    switch (key.e.key.sequence) {
      case (`\u001bb`): // ALT+b   ???(key.meta && key.name === 'b')???
        isBash=true
...
if (!config.browser.command) {
  log('warn', `(config.browser.command) not set, thus HTML content will not be rendered.`)
  config.browser.command = 'unavailable'
}  
if (!config.editor) {
  log('warn', `(config.editor) not set, thus 'vi' will be used for all buffered cache edits`)
  config.editor = 'vi'
}  
if (!config.ide) {
  log('warn', `(config.ide) not set, thus 'vi' will be used for all source file edits`)
  config.ide = 'vi'
}  

// inquirer prompt specifying the prompt as the question
module.exports = exports = cli = inquirer.prompt([
    log('progress', view.delimiter)
  ])
  .then( command => {
    switch (command) {
      case ("about"): async () => {
        async args => {
          ...
          switch (args.topic) {
            ...
            case ('ucons'):
              if (args.options.browse) {
                // dynamic markdown, whoo-hoo or boo-hoo?
...

{
  about: (topic, options) => {
    if (options)==='--help') {
      `about [topic]`, chalk`Documentation by topic\n
      ...
      `--browse` 
        `Open the document (as HTML) in a browser with '${config.browser.command}'`)
      `--edit`
        chalk`Edit the source document with 'config.editor' ({yellowBright Always overrides} {bold.inverse --browse})`)
    } 
  ...
  .action( async args => {
    if (await catalog.isHost()) {
      ...
      if (matchPattern) {
        ...
        result = await catalog.cmdAttached(containerId, `ls -lb ${folder}`)
...
vorpal.command(`repo [action]`, chalk`Source control using simple-git API\n
  Actions:  
    ...
    {bold fetch}          Bring all changes at remote into local repo
    {bold init}           Initialize a new local repo
    {bold merge}          Merge remote and local repo
    {bold reset}          Un-stage all staged files
    {bold rebase}         Rebase archive
...
.action( async (args) => {
  ...
  switch (args.action) {
    ...
    case ('merge'):
      ...
      break
    case ('push'):
      if (await api.confirm(`API push does not reset 'git statusSummary.ahead' counter\nBetter to use 'git push' at bash prompt instead.\nPush now anyway?`)) {
        await repo.push()
      }
      break
    case ('rebase'):
