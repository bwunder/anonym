          ...
          .then( async group => {
            ...
            if (action==='status') resolve(await api.shell(`ps -e|grep dockerd`)) 
              ...
              if (action.includes('start')) {
                switch (option) {
                  case ('all'):
      log('confirm', 'catalog.sql:') 
      log('confirm', catalog.sql) 
      log('confirm', `catalog.listInstanceIds('all'):`)              
      log('confirm', catalog.listInstanceIds('all'))              
                    for (const instanceId of catalog.listInstanceIds('all')) {
      log('confirm', `action ${action}, option ${option}, instance ${instanceId}`)
                  ...
                  case ('none'):
                    break
                  case ('running'):
                    for (const instanceId of catalog.listInstances('up')) {
                      log('confirm', `(setEngine) start instance: ${instanceId}`)
                      await catalog.startInstance(instanceId)
                    }
                    break
                  case ('target'):
                  default:
                    if (action.includes('start') && catalog.sql.ContainerInfos.get(catalog.sql.Instance).State==='running') {
                      log('confirm', `(setEngine) start target: ${catalog.sql.Instance}`)
