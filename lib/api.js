...
module.exports = exports = api = {
  ...
  attachDocker: async () => {
    ...
    return new Promise( async (resolve, reject) => {
      let cliOptions = {}
      let daemonOptions = {}
      let dockerAPIjson = path.resolve(config.docker.bindings.private.mount.Source, 'dockerAPI.json') 
      ...
      let dockerCert = path.resolve(config.docker.bindings.private.mount.Source, config.docker.daemon.cert)
      let dockerKey = path.resolve(config.docker.bindings.private.mount.Source, config.docker.daemon.key)
      let dockerCLICert = path.resolve(config.docker.bindings.private.mount.Source, config.docker.api.cert)
      let dockerCLIKey = path.resolve(config.docker.bindings.private.mount.Source, config.docker.api.key)
      if (!await api.isHost())
      ...
      if (!config.docker.daemon.tls) {
        if (daemonOptions.tls) {
          api.log('warn', chalk`(dockerAttach) {bold.italic.yellow dockerd TLS is de-escalated} engine stop/start required`) 
        ...
        cliOptions.port = 2375
        cliOptions.protocol = 'http'
      } else { //use tls 
        if (!fs.existsSync(dockerCert)) await api.genCertificate('docker', 'serverAuth', config.docker.daemon.password)
        if (!await api.isSigned(dockerCert)) reject(new Error(`(dockerAttach) dockerd certificate not signed by me\n${dockerCert}`))
        daemonOptions.tls = config.docker.daemon.tls
        daemonOptions.tlscacert = caCert
        daemonOptions.tlscert = dockerCert
        daemonOptions.tlskey = dockerKey
        cliOptions.port = 2376
        cliOptions.protocol = 'https'
        cliOptions.ca = fs.readFileSync(caCert)
        if (!config.docker.daemon.tlsverify) {
          cliOptions.cert = fs.readFileSync(dockerCert)
          cliOptions.key = fs.readFileSync(dockerKey)
          if (daemonOptions.tlsverify) {
            api.log('warn', chalk`(dockerAttach) {bold.italic.yellow dockerd TLSVERIFY 'clientAuth' is de-escalated}`) 
            delete daemonOptions.tlsverify
          }
        } else {
          if (!fs.existsSync(dockerCLICert)) await api.genCertificate('dockerCLI', 'clientAuth', config.docker.api.password)
          if (!await api.isSigned(dockerCLICert)) reject(new Error(`(dockerAttach) dockerCLI certificate not signed by me\n${dockerCLICert}`))
          daemonOptions.tlsverify=config.docker.daemon.tlsverify
          cliOptions.cert = fs.readFileSync(dockerCLICert)
          cliOptions.key = fs.readFileSync(dockerCLIKey)
        } 
      }                 
      daemonOptions.hosts = []
      for (let IP4 of await api.listHostIP4s()) {
        if (!cliOptions.host) cliOptions.host = IP4
        daemonOptions.hosts.push(`tcp://${IP4}:${cliOptions.port}`)
      }
      await api.JSONtoFile(JSON.stringify(daemonOptions, null, 2), daemonjson) 
      await api.JSONtoFile(JSON.stringify(cliOptions, null, 2), dockerAPIjson) 
      if ((await api.shelevate(`diff ${daemonjson} /etc/docker/daemon.json`)!=='')) {
        await api.shelevate(`cp ${daemonjson} /etc/docker/daemon.json`)
        api.log('warn', chalk`(dockerAttach) {bold.italic.yellow dockerd TLS configuration is changed}
          a stop and start of the Docker daemon may be required to effect this change. A restart
          will not change the httpd socket type or protocol. Be aware that stopping the daemon will stop
          all contained SQL Servers but does not restart them when started. Only a service restart works 
          that way with systemd, but even then, all connections are closed and not automtically re-attended.
          The cli will reconnect only to the target once the containers are restarted and ready.`) 
      ...
      api.docker = new Docker(cliOptions)
      resolve(true)
    })

  },  
  bandAid: () => {
    return chalk`{rgb(0, 127, 255) \u250F}                       {rgb(255, 0, 127) \u23AB}\n` +
      chalk`{rgb(0, 127, 255) \u2503}  {bold.italic ${api.name}}  {green \u2316}  v. ${version}  {rgb(255, 0, 127) \u23AC}\n` +
      chalk`{rgb(0, 127, 255) \u2517}                       {rgb(255, 0, 127) \u23AD}\n`
  },
  batch: [''],
  checkNPM: async () => {
  ...
  cliAid: () => {
    ...
    let tab = chalk.bold(`\u276F`)
    return chalk`\n\n{bold ${api.name}}\n\tfor Architects, Developers, Tester & other Evaluators\n\n
    USAGE: 
    [ {bold t-sql...} | [[{bold command}|startsWith${tab}]${tab}${tab}]  [{bold --}[{bold option}|startsWith${tab}]${tab}${tab}][ ...]  [[{bold argument}|startsWith${tab}]${tab}${tab}][ ...] ]
      ...
      Help for all CLI commands 
        {italic.bold ${'help [command]'.padEnd(30)}}  Command usage else a descriptive index of all commands when no command arg.
        {italic.bold ${'[command] --help'.padEnd(30)}}  Command usage else CLI usage when no command (same as {bold.italic about cli}).

      Commonly needed next commands at startup:
        {italic.bold ${'engine start'.padEnd(30)}}  Start the Docker Container Engine Instance
        {italic.bold ${'container start'.padEnd(30)}}  Start an existing SQL Server Instance
        {italic.bold ${'catalog'.padEnd(30)}}  Review the Inventory of local SQL Server Images and Containers
        {italic.bold ${'image pull'.padEnd(30)}}  Fetch '${config.docker.pull.repo}:latest' from dockerhub
        {italic.bold ${'image run'.padEnd(30)}}  Instantiate SQL Server instance from a local image
        {italic.bold ${'container target'.padEnd(30)}}  Target CLI queries at an existing SQL Server Container
        {italic.bold ${'batch'.padEnd(30)}}  TSQL entered and cached since last query execution on a Target SQL Server
        {italic.bold ${'go'.padEnd(30)}}  Submit all TSQL in cache for execution by current Target SQL Server\n\n`
  },
  ...
  commandAid: (cmds) => {
    ...
    return [
      chalk`
      Deployment Automation and Lifecycle Tools for hosted SQL Server Containers 
      ...
      Architect, Develop, Test & otherwise evaluate Private & Autonomous Virtual Data Topologies   
        ...
        appended to an edit-able buffer cache as text. This {bold.italic Batch} cache anticipates that the input 
        is valid TSQL. Any time the user is uncertain that is true, the Batch cache can be parsed by a SQL Server 
        query engine with the {bold.italic issql} command. {bold.italic issql} is non-destructive to the cache. 
        ...
        Cached TSQL can be cleared from cache with {bold.italic batch reset} - or, using autocomplete and the '?' 
        alias for the Batch command - the same command to clear the Batch cache can be entered as {bold.italic ? r${tab}}.  
        When {rgb(255, 255, 0) Terminating} commands discussed below are entered, the {bold.italic Batch} cache is compiled to one 
        query string per SQL Batch and submitted for execution by the targeted SQL Server. 
        ...
        Valid CLI Commands can be entered at any new line, regardless if the Batch cache holds text*. Commands that 
        do not use the cache don't mess with cache content. Only {bold.italic query}, {bold.italic script}, the terminating commands 
        and the {bold.italic batch} command use and affect the Batch cache.
        
        ${chalk.underline.rgb(255, 255, 0)(`Terminating commands`)} submit the Batch to Query Engine of Target SQL Server
        ${term.join('\n\t')}

        All CLI commands and options - including {bold.italic go} & {bold.italic use} of particulare note - are lower case and case 
        sensitive. All UPPERCASE or MixedCase text goes into the Batch cache.
        
        Only the {bold.italic run} or {bold.italic sqlcmd} commands can process a batch that includes upper or mixed case 'GO' or 
        'Use' directives - that, while generally thought of as TSQL, are always client side preprocessed. 
        
        When the Batch is submitted, by default the {bold.italic issql} check is first submitted to help mitigate 
        ...
        Command index: {bold.italic help} alias (no dashes) 
        Usage: {bold.italic about quickstart} or, using autocomplete, {bold.italic a${tab} q${tab}} for to get
        ...
        *Should the first word of a valid TSQL line conflict with a command name, the command has precedence and 
        ...
        the query using {bold.italic batch edit} or re-style the TSQL to avoid the start of line conflict.           
      `]
  },
  compile: metadata => {
  ...
  genSigningRequest: async name => {
    ...
    return new Promise( async (resolve, reject) => {
      try {
        // please donate to email shown on paypal before changing this, thank you  
...
api.log('confirm', `deleting file ${file}`)
          fs.unlinkSync(file)
        }
  ...
  interactiveShell: (containerId, command='') => {
    ...
    return new Promise( (resolve, reject) => {
      try {
       //  bcp & sqlcmd in the current $PATH. and then there is $MSSQL_SA_PASSWORD 
       if (containerId) {
          let container = api.docker.getContainer(containerId)           
          let options = {
            "AttachStdin": true,
            "AttachStdout": true,
            "AttachStderr": true,
            ...
            "Tty": true,
            "Cmd": [`ln -sf ${config.cli.odbc.path}/sqlcmd /usr/bin; ln -sf ${config.cli.odbc.path}/bcp /usr/bin; ${command}`],
            "Env": []
          }
  ...
  intern: async instanceId => {
    ...
    try {
      if (await api.attachDocker()) {
        api.log('log', `\u2042 mapping host's SQL Container catalog\n`)
        ...
        if (await api.internImages()) {
          ...
          if (api.sqlCatalog.ContainerInfos.has(instanceId)) {
            ...
            if (instanceId===api.sqlCatalog.Instance) { // this is false when the engine is off
              if (api.sqlCatalog.ContainerInfos.get(instanceId).State==='running') {
                await api.openInstance(instanceId)
              } else {
                api.log('log', chalk`\u2042 {yellow Target SQL Container is not 'running'}, try {italic.bold container start}`)
              }
            } else {
              api.log('log', chalk`\u2042 {yellow Targeted SQL Server not found}, ${instanceId}`)
            }
          } else {
            api.log('log', chalk`\u2042 {yellow No Local SQL Server Containers}`)
          }
        } else {
  ...
  log: (mode, data) => {
    ...
    try {
      // will have chalk marks (color codes) - not so good in for a file, console or nedb
      if (['confirm', 'debug', 'error', 'info', 'log', 'test', 'warn'].includes(mode)) {
        switch (mode) {
        case ('confirm'): // 2611 - check in box  
        ...
        case ('test'):
          ...
          break
        case ('warn'): // 26A0 26DB 2bc6 2BC6 ${"\u{1F53B}"}
  ...
  renameInstance: async (containerId, name) => {
    ...
    if (api.sqlCatalog.ContainerInfos.has(containerId)) {
      await api.docker.getContainer(containerId).rename(name)
    }
    ...
    return new Promise( async(resolve, reject) => {
      try {
        ...
        sqlpad.sqlpad.stdout.on('data', data => {
          if (/Welcome/.test(data)) {            
            ...
            api.log('log', chalk.cyan.italic(data) +
               chalk`\n{italic.bold (sqlpad)} browser wih V8 is required (e.g., Chrome or Chromium)`)
            resolve()
          } else {
            if (sqlpad.debug) {
        ...
        sqlpad.sqlpad.on('exit', code => {
          api.log('warn', chalk`{cyan.italic (sqlpad)} {gray server has exited} code: ${code||0}`)
          sqlpad.sqlpad={}
          resolve()
        })  
      }
