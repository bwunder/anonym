...
module.exports = exports = api = {
  ...
  cliAid: () => {
    return chalk`
      ...
      {italic.bold ${'container --target [container-id]'.padEnd(40)}} Target your queries at a SQL Server Container
      {italic.bold ${'catalog'.padEnd(40)}} Catalog of Local SQL Server Images, Containers and Connection Pools
      {italic.bold ${'container --start  [container-id]'.padEnd(40)}} Start a SQL Server Container
    ...
    return new Promise(async function(resolve, reject) {
      try {
        ...
        if ((await api.elevate(`ls ${caCert}`)).status===0 &&
          ...
          await api.elevate(`openssl genrsa -aes256 -out ${key} 2048`)
          if ((await api.elevate(`ls ${key}`)).status===0) {
            await api.elevate(`openssl req -new -config ${cnf} -key ${key} -out ${signingReq}`)
          }
