...
const util = require('util') // unqualified inspect can conflict with
////local
const { chooseInstanceId, intern, isHost, latestImageId, listInstanceIds, setDockerRemoteAPI, sql, startInstance } = require('../lib/catalog')
...
// extensive self-referencing (api.)
module.exports = api = {
  addFolder: async dir => {
  ...
  attachDocker: async () => {
    return new Promise( async (resolve, reject) => {
      try {
        let currentOptions // set in try, need out here for scope
...
// TODO !!! try fetch configs from store first... because.... (note has aged beyond my recollection)               
        ...
        address = (running.length>0)?networkInterfaces()[config.docker.networkInterface].address: config.docker.daemon.socketPath
        api.apiOptions = await setDockerAPI(address) 
        api.cliOptions = await setDockerShell(address)
        const daemonOptions = await setDockerDaemon(address)
        try {
  ...
  choose: (choiceArray, message='Select one', suggestedResponse) => {
    ...
    return new Promise( async (resolve, reject) => {
      ...
      .then( answer => resolve(answer.choice))
  ...
  fileToBatch: async (scriptFile) => {
    return new Promise( (resolve, reject) => {
      try {
        readFile(path.resolve(scriptFile), 'utf8', (err, script) => {
          if (err) reject(err)
          if (script) {
            api.batch.splice(0)
  ...
  getTimestamp: () => { 
    // a valid datetime string with, no white space, could be used as file name
  ...
  interactiveShell: (containerId, command) => {
    return new Promise( async (resolve, reject) => {
      try {
        ...
        if (containerId) {
          log('log', chalk`Connecting (perhaps as 'root', more likely as 'mssql' - whoami) to SQL Server container ${containerId}
          ...
          Type {bold exit} to end close command line and resume node event loop.`)
  ...
  listFiles: async (folder, filter) => {
    return new Promise( async (resolve, reject) => {
      try {
        const list=[]
        readdir(path.resolve(folder), {"encoding":'utf8', "withFileTypes": true} , (err, files) => {
          if (err) reject(err)
          files.forEach( file => {
            if (!filter || file.name.includes(filter)) {
              list.push(file.name)
            }
          })
...
// ???not in use??? 
            ...
            if (action==='status') resolve(await api.shell(`ps -e|grep dockerd`)) 
              ...
              if (action.includes('start')) {
                switch (option) {
                  ...
                  case ('target'):
                  default:
                    if (sql.Instance && sql.ContainerInfos.get(sql.Instance).State==='running') {
  ...
  txtToFile: async (txt, toFile) => { // makes good .html -vs- jsonToFile's inspect(obj)
    return new Promise( async (resolve, reject) => {
      try {
        writeFile(path.resolve(toFile), txt, 'utf8', (er) => er? reject(er): resolve())
      }
      catch(err) {
        reject(err)
      }
