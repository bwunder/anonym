        ...
        return api.shell(`sudo getent group docker`)
          .then( async group => {
            ...
            if (action==='status') resolve(await api.shell(`ps -e|grep dockerd`)) 
              ...
              switch (option) {
                case ('all'):
                  log('confirm', `option ${option}`)
                  for (const instanceId of catalog.listInstances(action.includes('restart')? 'up': 'all')) {
                    await catalog.startInstance(instanceId)
                  }
                  break
                case ('target'):
                  log('confirm', `option ${option} ${catalog.sql.ContainerInfos.get(catalog.sql.Instance).State}`)
                  if (action.includes('start') && catalog.listInstances('up').includes(catalog.sql.Instance)) {
