...
module.exports = exports = api = {
  ...
  commandAid: (cmds) => {
    ...
    return [
      chalk`\tEntered text is processed as a {bold.italic ${package.name}} CLI command else accumulated into the Batch - an array of text lines.
      ...
      \n\tUPPERCASE 'GO' and UPPERCASE 'USE' are treated as valid inline tSQL and are not processed as CLI commands
      \n\tIt is recommended to always copy-paste Docker Object Ids from a {italic.bold catalog} view when needed as a CLI command arg` ]
  ...
  dockerAid: () => {
    return chalk`
      {italic.bold ${'image    --pull'.padEnd(40)}} get '${config.docker.repo}:latest' from dockerhub
  ...
  developFile: async (textFile) => {
    ...
    if (!textFile.endsWith('sql')) {
      api.log('log', `Saved changes may not be recognized until ${package.name} is restarted`)
    }
  ...
  editText: async (text) => {
    ...
    return new Promise(function(resolve, reject) {
      if (!config.cli.editor) {
        api.log('warn', `editor not configured? using vim...`)
        config.cli.editor = 'vim'
      }
      api.log('log', `opening for edit with '${config.cli.editor}'...`)
  ...
  genCA: () => {
    ...
    return new Promise( async function(resolve, reject) {
      let caKey = path.resolve(config.cli.privatePath,`CA-key.pem`)
      let caCert = path.resolve(config.cli.privatePath,'CA-cert.pem')
      try {
        await api.elevate(`ls -dH ${config.cli.privatePath}`)
//        fs.accessSync(config.cli.privatePath, fs.constants.F_OK);
      } catch (err) {
        if (err.code === 'ENOENT') {
          api.log('log', `creating link from ${path.resolve(config.cli.privatePath)} to ${config.cli.privateLink}`)
          await api.elevate(`ln -s ${config.cli.privateLink} ${path.resolve(config.cli.privatePath)}`)
//          fs.mkdirSync(config.cli.privatePath)
        } else {
          api.log('error', err)
        }
      }
  ...
  genCertificate: (name) => {
    ...
    return new Promise(async function(resolve, reject) {
      try {
        let caKey = path.resolve(config.cli.certPath,`CA-key.pem`)
        let caCert = path.resolve(config.cli.certPath, 'CA-cert.pem')
        let key = path.resolve(config.cli.certPath, `${name}-key.pem`)
        let signingReq = path.resolve(config.cli.certPath, `${name}-signingReq.csr`)
        let cert = path.resolve(config.cli.certPath, `${name}-cert.pem`)
        let cnf =  path.resolve(config.cli.certPath, `openssl.cnf`)
    ...
    return new Promise(function(resolve, reject) {
      ...
      if (containerId) {
        childProcess.execSync(`docker exec -d ${containerId} /bin/bash
          docker exec -d ${containerId} ln -sf ${config.cli.odbcPath}/sqlcmd /usr/bin
          docker exec -d ${containerId} ln -sf ${config.cli.odbcPath}/bcp /usr/bin
        ...
        api.log('info', [`Entering interactive session \n\tSQL Server container ${containerId}...`,
          ...
          `Type 'exit' to close interactive session and resume ${package.name} prompt at host`].join('\n'))
...
// bwunder@linux-niun:~/sqlpal> ln -s /var/lib/docker/volumes/private/_data private
  ...
  startSQLPad: () => {
      ...
      pem.createCertificate(config.pem, function(err, keys){
        ...
        api.log('confirm', `(sqlpad) generate new self-signed SSL Certificate`)
// need root to write to docker volume target path
// these don't even need to go out to  there for this use but aslo need this access to pass SQL TLS creds
        fs.writeFileSync(path.resolve(config.sqlpad[`key-path`]), keys.serviceKey)
        fs.writeFileSync(path.resolve(config.sqlpad[`cert-path`]), keys.certificate)
        ...
        // something not right - while logged in as admin on host I still see "Admin Registration is  Open" in browser signup???
