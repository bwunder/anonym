  ...
  attachDocker: async () => {
    return new Promise( async (resolve, reject) => {
      try {
        ...
        const running = catalog.listInstanceIds('up') 
        // get CA ready
        return tls.genCA()
        .then( async () => {
        ...
        .then( async () => {
// try fetch configs from store here          
          let address = config.docker.daemon.socketPath
          if (running.length>0) {
             address = networkInterfaces()[config.docker.networkInterface].address
          } 
          api.apiOptions = await tls.setDockerAPI(address) 
          api.cliOptions = await tls.setDockerShell(address)  
          daemonOptions = await tls.setDockerDaemon(address)
          try {
            currentOptions = await api.fileToJSON(daemonCfg)
          }
          catch (err) {
            if (err.code==='ENOENT') log('warn', 'no previous daemon.json') 
            else reject(err)
          }
          if (daemonOptions) {
          ...
          if ((daemonOptions && !currentOptions) || daemonOptions!==currentOptions) {
            ...
            log('warn', chalk`(attachDocker) {bold.yellowBright Restarting Container Instance Engine - TLS changes detected.}`) 
            if (await catalog.isHost()) {
              await api.setEngine('stop')
            }
            await api.setEngine('start')
            catalog.setDockerRemoteAPI(api.apiOptions)      
          }  
        }) 
        .then( async () => {
...
// save configs to store here          
  ...
  listDatabases: async (containerId=catalog.sql.Instance) => {
    let databases = []
    let results = await sqldb.query('select name from sysdatabases', containerId)
        ...
        return catalog.getProcesses(containerId)
          .then( async (top) => {
            if (top) {
              ...
              poolConfig.database = await sqldb.openPool(containerId, poolConfig)
  ...
  spawnTask: async (bashCommand) => { 
    return new Promise( async (resolve, reject) => {
      try {
        ...
        submit.unref()
        resolve(log('confirm', `(spawnTask) Submitted bashCommand: ${bashCommand}`))
      }
      catch(er) {
        reject(er)
      }  
